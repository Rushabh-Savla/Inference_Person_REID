...
def _load_checkpoint(*a, **k):
    raise RuntimeError("pretrained backbone loading disabled -- PRETRAIN_PATH is blank by design")
...
class ShiftWindowMSA(BaseModule):
    ...
    def window_reverse(self, windows, H, W):
        ...
        # -1 for the batch, NOT int(windows.shape[0] / ...): int() on a
        # traced tensor freezes the batch into an ONNX graph, so the export is
        # only correct at the size it was traced with. C is read explicitly
        # because a view can carry only one -1.
        C = windows.shape[-1]
