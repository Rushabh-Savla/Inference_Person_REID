...
class BatchPipelineStateAuthoritative(BatchPipelineStateInvariantJointAttributes):
    """Final accuracy-first MTMC path with hard overlap isolation and recovery."""
    ...
    def __init__(self, config_path: str):
        ...
        self.overlap_exit = float(
            self.cfg.get(
                "authoritative_overlap_exit",
                guard.get("authoritative_overlap_exit", 0.58),
            )
        )
    ...
    def _overlaps(self, items):
        """Detect sustained close overlap/occlusion with hysteresis."""
        blocked = set()
        partners = {}
        seen_pairs = set()
        ...
        for i in range(len(items)):
            for j in range(i + 1, len(items)):
                ...
                meaningful = bool(
                    iou >= self.occlusion_iou
                    and iom >= self.occlusion_intersection
                )
                signal = max(iou, iom)
    ...
    def add_body(self, key, camera, track_id, segment, bbox, stamp, score, image, feats, multi):
        ...
        meta = self._pending_recovery.get(key)
        if not meta:
            return
        track = self.tracks.get(key)
        if track is not None:
            setattr(track, "overlap_recovery", True)
            setattr(track, "recovery_sources", list(meta.get("sources", [])))
            setattr(track, "recovery_camera", str(meta.get("camera", camera)))

    def _extract(self, camera, frame, fps, image, prepared, blocked, partners, info, rows):
        """Hard-freeze overlap observations and force a clean post-overlap segment."""
        for item in prepared:
            ...
            if active and not previous:
                ...
                old_refs = self._refs.get(
                    previous_key,
                    {"resnet": [], "swin": [], "solider": []},
                )
                self._refs[key] = {
                    model: list(old_refs.get(model, []))
                    for model in ("resnet", "swin", "solider")
                }
                self._fails[key] = 0
                self._pending_recovery[key] = {
                    "camera": camera,
                    "sources": [previous_key],
                }
                info["recovery_left"][key] = self.recovery_samples
                info["last"][key] = -10**9
                boundary = True
                reason = "authoritative_overlap_exit_new_segment"
            ...
            if self.light and (
                due
                or frame - info["last"].get(key + ":light", -10**9) >= self.part_interval
            ):
