...
class BatchPipelineStateInvariant(BatchPipelineV6):
    ...
    def add_body(self, key, camera, track_id, segment, bbox, stamp, score, image, feats, multi):
        ...
        super().add_body(
            key, camera, track_id, segment, bbox, stamp, measured, image, feats, multi
        )
        ...
        for kind, vector in feats.items():
            if kind in track.state_bank["resnet"]:
                ...
                track.state_bank["resnet"][kind] = track.state_bank["resnet"][kind][-96:]
        ...
        for model in ("swin", "solider"):
            for kind, vectors in multi.get(model, {}).items():
                ...
                track.state_bank[model][kind] = track.state_bank[model][kind][-96:]

        signature = self.colour_signature(image)
        if signature is not None:
            track.colour_bank.append(signature)
            track.colour_bank = track.colour_bank[-48:]
        ...
        try:
            while True:
                ...
                for item in detector.track(image):
                    ...
                    self.add_body(key, camera, tid, seg, frame / fps, box, float(item.confidence), person, resnet_feats, multi) if False else self.add_body(key, camera, tid, seg, box, frame / fps, float(item.confidence), person, resnet_feats, multi)
                    samples += 1
                    feature_batches += 1
        finally:
            cap.release()
    ...
    def run(self, values):
        ...
        try:
            ...
            print(f"[state-final] accepted same-camera repairs: {same_count}")
            print(f"[state-final] accepted cross-camera links: {cross_count}")
