...
from rebuild.identity_v2 import (
...
class BatchPipelineV2:
    """V2 baseline retained for comparison only.

    Final rendering intentionally shows only the persistent Global ID. V3 is the
    default production batch pipeline.
    """
    ...
    def __init__(self, config_path: str):
        ...
        self.extractor = ReIDExtractor(weights=reid["weights"], device=device,
                                       max_batch=int(reid.get("max_batch", 32)), model=reid.get("model"))
    ...
    def run(self, values: List[str]) -> Dict[str, str]:
        ...
        print("[v2] pass 1: detect + track + collect embeddings")
        for camera, path in sources:
            self.collect_camera(camera, path)
        self.save_cache()
        print("[v2] pass 2: global reconciliation")
        ...
        print("[v2] pass 3: render from saved detections")
        self.render(mapping)
    ...
    def collect_camera(self, camera: str, path: str) -> None:
        ...
        detector = PersonDetector(model_path=self.detector_cfg["model"],
                                  confidence_threshold=float(self.detector_cfg["conf"]),
                                  person_class_id=0, tracker_config=self.detector_cfg["tracker"],
                                  pose_ensemble=None, iou=float(self.detector_cfg["iou"]))
        ...
        try:
            while True:
                ...
                for item in detections:
                    ...
                    events.write(json.dumps({"camera": camera, "frame": frame_index,
                                             "timestamp": frame_index / fps, "track_id": track_id,
                                             "tracklet_key": key, "bbox": list(bbox),
                ...
                if crops:
                    ...
                    for key, track_id, segment, bbox, q, det_score in meta:
                        ...
                        row = {"camera": camera, "frame": frame_index, "timestamp": frame_index / fps,
                               "track_id": track_id, "bbox": list(bbox), "detection_score": det_score,
                               "quality": q}
                        tracklet.add(original, q, row)
                        ...
                        if self.use_light:
                            ...
                            if agreement < 0.995:
                                tracklet.add(light, max(0.10, q * 0.92), {**row, "variant": "illumination"})
    ...
    def save_cache(self) -> None:
        ...
        for key, track in sorted(self.tracklets.items()):
            rows.append({"key": key, "camera": track.camera, "track_id": track.track_id,
                         "segment": track.segment, "fps": track.fps, "start": track.start,
                         "end": track.end, "embedding_count": track.count,
                         "quality": track.embedding_quality, "bbox_aspect": track.shape})
            if track.embeddings:
    ...
    def save_mapping(self, mapping: Dict[str, str], matches) -> None:
        ...
        with (self.out / "identity_matches_v2.jsonl").open("w", encoding="utf-8") as handle:
            for match in matches:
                handle.write(json.dumps({"left": match.left, "right": match.right,
                                         ...
                                         "score": match.score, "margin_left": match.margin_left,
                                         "margin_right": match.margin_right, "reciprocal": match.reciprocal,
                                         "relation": match.relation, "decision": "MERGE"}) + "\n")
    ...
    def save_gallery(self, mapping: Dict[str, str]) -> None:
        ...
        meta = {gid: {"members": [key for key, value in mapping.items() if value == gid],
                      "prototype_count": int(len(gallery)), "embedding_dim": int(gallery.shape[1])}
    ...
    def render(self, mapping: Dict[str, str]) -> None:
        for camera, meta in self.video_meta.items():
            ...
            writer = cv2.VideoWriter(str(output), cv2.VideoWriter_fourcc(*"mp4v"), meta["fps"],
            ...
            try:
                while True:
                    ...
                    for record in frame_rows.get(frame_index, []):
                        ...
                        cv2.putText(frame, gid, (x1, max(25, y1 - 8)), cv2.FONT_HERSHEY_SIMPLEX,
                                    0.65, (0, 255, 0), 2, cv2.LINE_AA)
                    cv2.putText(frame, camera, (20, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.9,
