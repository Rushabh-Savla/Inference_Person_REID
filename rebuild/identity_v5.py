...
class GlobalIdentityV5:
    """V3 body matcher with adaptive multi-observation evidence and face rescue."""
    ...
    def conflict(self, query: Tracklet, identity: Identity, tracks: Dict[str, Tracklet]) -> bool:
        ...
        return any(
            ...
            and item.camera == query.camera
            and self.overlap(query, item)
    ...
    def pair(left: List[Feature], right: List[Feature]) -> Tuple[float, int]:
        ...
        return float(0.85 * values[0] + 0.15 * values[min(1, len(values) - 1)]), int(sum(x[1] for x in parts))
    ...
    @staticmethod
    def face_quality(values: List[Feature]) -> float:
    ...
    def search(self, track: Tracklet, faces: List[Feature], tracks: Dict[str, Tracklet]) -> List[dict]:
        ...
        for gid, identity in self.identities.items():
            ...
            rows.append({"gid": gid, "body": body, "support": max(pair_support, obs_support), "obs_support": obs_support,
                         "stable_support": stable_support, "face": 0.0, "face_support": 0, "face_margin": 0.0})

        usable_face = bool(faces) and self.face_quality(faces) >= self.face_quality
        ranked_faces = []
        if usable_face:
            for row in rows:
                values = self.face_trusted.get(row["gid"], []) + self.face_candidate.get(row["gid"], [])
                score, support = self.face_pair(faces, values)
                row["face"], row["face_support"] = float(score), int(support)
                ranked_faces.append((float(score), row["gid"]))
            ranked_faces.sort(reverse=True)
            for i, (value, gid) in enumerate(ranked_faces):
                second = ranked_faces[i + 1][0] if i + 1 < len(ranked_faces) else 0.0
                for row in rows:
                    if row["gid"] == gid:
                        row["face_margin"] = float(value - second)
                        break
    ...
    def accept(self, best: dict, second: float, faces: List[Feature]) -> Tuple[bool, str]:
        ...
        fq, fm = self.face_quality(faces), best["face_margin"]
        ...
        if face >= self.face_threshold and fq >= self.face_quality and fm >= self.face_margin and body >= 0.54 and margin >= 0.02:
        ...
        if best is not None:
            ...
            if accepted:
                ...
                trusted = bool((best["body"] >= self.strong and best["support"] >= self.support) or
                               (best["face"] >= self.face_rescue and self.face_quality(faces) >= self.face_strong_quality))
        ...
        while changed:
            ...
            for i, left_gid in enumerate(gids):
                ...
                for right_gid in gids[i + 1:]:
                    ...
                    faces_l = self.face_trusted.get(left_gid, []) + self.face_candidate.get(left_gid, [])
                    faces_r = self.face_trusted.get(right_gid, []) + self.face_candidate.get(right_gid, [])
                    face, _ = self.face_pair(faces_l, faces_r)
                    if (body >= self.merge_body and support >= self.merge_support) or face >= self.merge_face:
                        ...
                        self._merge(left_gid if left_size >= right_size else right_gid,
                                    right_gid if left_size >= right_size else left_gid)
                        changed = True
                        break
                if changed:
                    break
    ...
    def run(self, tracks: Dict[str, Tracklet], faces_by_track: Dict[str, List[Feature]]):
        ...
        for track in sorted(tracks.values(), key=lambda x: (x.evidence(), x.start, x.camera, x.key)):
            ...
            if not ranked:
                continue
            best, second = ranked[0], ranked[1]["score"] if len(ranked) > 1 else 0.0
    ...
    def summary(self, tracks: Dict[str, Tracklet]) -> dict:
        ...
        return {
            "tracklets": len(tracks), "global_ids": len(set(self.mapping.values())),
            "multi_camera": multi, "multi_camera_count": len(multi), "reasons": reasons,
            ...
            "score_buckets": buckets, "identity_merges": self.merge_count,
