...
class GlobalIdentityV6:
    ...
    Unlike V5, V6 does not create a permanent Global ID as soon as a tracklet is
    insufficiently matched. It first establishes track-to-track relationships,
    explicitly models recent same-camera track loss/re-entry, forms provisional
    identity components, and only then assigns canonical Global IDs.
    """
    ...
    def pair_edge(self, left: Tracklet, right: Tracklet, faces: Dict[str, List[Feature]]) -> EdgeV6 | None:
        ...
        return EdgeV6(
            ...
            "+".join(reason_parts) or "appearance",
        )
    ...
    def build_edges(self, tracks: Dict[str, Tracklet], faces: Dict[str, List[Feature]]) -> None:
        ...
        for i, left in enumerate(values):
            for right in values[i + 1:]:
                if left.camera == right.camera and right.start - left.end > self.same_camera_gap:
                    # Same-camera continuity is only useful within the short lost-track window.
                    pass
    ...
    def _cluster(self, tracks: Dict[str, Tracklet]) -> Dict[str, List[str]]:
        ...
        for edge in self.edges:
            ...
            if any(self._component_overlap(left_members, tracks[r], tracks) for r in right_members):
                continue
            # Prevent weak transitive bridges: every new edge must be either a
            # strong appearance edge or carry continuity/face support.
    ...
    def _add_face(self, gid: str, values: List[Feature], trusted: bool) -> None:
        if not values:
            return
        bank = self.face_trusted.setdefault(gid, []) if trusted else self.face_candidate.setdefault(gid, [])
        for item in values:
            if item.quality < self.promote and trusted:
                continue
            if bank:
    ...
    def _cluster_score(self, members: List[str], tracks: Dict[str, Tracklet], faces: Dict[str, List[Feature]]) -> Tuple[float, float, int]:
        body_scores = []
        face_scores = []
        supports = []
        for i, left_key in enumerate(members):
            for right_key in members[i + 1:]:
                left, right = tracks[left_key], tracks[right_key]
                body, support = self._safe_pair(left.features, right.features)
                face, fs = self._face_pair(faces.get(left_key, []), faces.get(right_key, []))
                if body > 0:
                    body_scores.append(body)
                if face > 0:
                    face_scores.append(face)
                supports.append(max(support, fs))
        return (
            float(max(body_scores) if body_scores else 0.0),
            float(max(face_scores) if face_scores else 0.0),
            int(max(supports) if supports else 0),
        )
    ...
    def run(self, tracks: Dict[str, Tracklet], faces: Dict[str, List[Feature]]):
        ...
        # Identity components are already merges. This second pass provides a
        # deterministic merge accounting record for components containing >1 track.
        for members in components.values():
            if len(members) <= 1:
                continue
            gid = self.mapping[members[0]]
            self.identity_merges += max(0, len(members) - 1)
