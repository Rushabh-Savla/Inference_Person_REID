...
class StateInvariantFinalResolver:
    ...
    def _best_state_score(cls, left: Mapping[str, List[np.ndarray]], right: Mapping[str, List[np.ndarray]]) -> Tuple[float, int, bool]:
        ...
        base = 0.0
        if direct:
            base = float(np.mean(sorted(direct, reverse=True)[:2]))
    ...
    def _mutual_models(self, left: LocalGroup, right: LocalGroup, left_pool: List[LocalGroup], right_pool: List[LocalGroup]) -> int:
        ...
        for model in self.MODELS:
            ...
            if not row_scores or not col_scores:
                continue
            row_idx = int(np.argmax(row_scores))
            col_idx = int(np.argmax(col_scores))
    ...
    def _group_from_track(cls, key: str, track, local_gid: str) -> LocalGroup:
        ...
        return LocalGroup(
            ...
            state_bank={
                model: {view: list(values) for view, values in views.items() if view in cls.VIEWS}
    ...
    def _merge_groups(left: LocalGroup, right: LocalGroup) -> LocalGroup:
        ...
        colours = []
        if left.colour_signature is not None:
            colours.append(np.asarray(left.colour_signature, np.float32))
        if right.colour_signature is not None:
            colours.append(np.asarray(right.colour_signature, np.float32))
        colour = None
        if colours:
        ...
        return LocalGroup(
            key=f"{first.key}+{second.key}", camera=first.camera, local_gid=first.local_gid,
            members=first.members + second.members, start=min(first.start, second.start), end=max(first.end, second.end),
            aspect=aspect, state_type=first.state_type if first.state_type == second.state_type else "mixed",
            state_bank=bank, colour_signature=colour, start_center=first.start_center, end_center=second.end_center, end_height=second.end_height,
        )

    @classmethod
    def build_groups(cls, local_mapping: Mapping[str, str], tracks: Mapping[str, object]) -> List[LocalGroup]:
        return sorted([
            cls._group_from_track(key, track, str(local_mapping.get(key, "UNKNOWN")))
            for key, track in tracks.items()
            if getattr(track, "observations", None)
        ], key=lambda g: (g.camera, g.start, g.key))
    ...
    def same_camera_edges(self, groups: List[LocalGroup]) -> List[dict]:
        if len(groups) < 2:
            return []
        ordered = sorted(groups, key=lambda g: (g.start, g.end, g.key))
        ...
        for i, left in enumerate(ordered):
            for j in range(i + 1, len(ordered)):
                ...
                if right.start - left.end > self.same_max_gap:
                    break
                item = self.pair(left, right, ordered, ordered)
                threshold = self.partial_min if item.state_transition else self.same_min
                if item.fused >= threshold:
                    matrix[(i, j)] = item.fused
                    evidence[(i, j)] = item
        by_left: Dict[int, List[Tuple[int, float]]] = defaultdict(list)
        by_right: Dict[int, List[Tuple[int, float]]] = defaultdict(list)
        for key, score in matrix.items():
            by_left[key[0]].append((key[1], score))
            by_right[key[1]].append((key[0], score))
        edges = []
        for (i, j), _score in sorted(matrix.items(), key=lambda item: item[1], reverse=True):
            row = sorted(by_left[i], key=lambda x: x[1], reverse=True)
            col = sorted(by_right[j], key=lambda x: x[1], reverse=True)
            row_margin = row[0][1] - row[1][1] if len(row) > 1 else 1.0
            col_margin = col[0][1] - col[1][1] if len(col) > 1 else 1.0
            item = evidence[(i, j)]
            if not self._same_accept(item, float(row_margin), float(col_margin)):
                continue
    ...
    def _stitch(groups: List[LocalGroup], edges: List[dict]) -> List[LocalGroup]:
        ...
        for edge in sorted(edges, key=lambda item: item["fused"], reverse=True):
            ...
            if a == b or StateInvariantFinalResolver._overlap(lookup[a], lookup[b]):
                continue
            union(a, b)
    ...
    def _cross_accept(self, evidence: PairEvidence, row_margin: float, col_margin: float) -> bool:
        ...
        margin = self.partial_margin if evidence.state_transition else self.cross_margin
        if evidence.fused < threshold or row_margin < margin or col_margin < margin:
            return False
        if evidence.model_support < 2 or evidence.mutual_models < 2 or evidence.agreement < 0.55:
            return False
        strongest = max(evidence.resnet, evidence.swin, evidence.solider)
        if evidence.state_transition and evidence.view_support < 3 and strongest < self.strong:
            return False
        return True
    ...
    def cross_edges(self, left: List[LocalGroup], right: List[LocalGroup]) -> List[dict]:
        ...
        for i, a in enumerate(left):
            for j, b in enumerate(right):
                ...
                details[(i, j)] = item
                threshold = self.partial_min if item.state_transition else self.cross_min
                if item.fused >= threshold:
                    matrix[(i, j)] = item.fused
        by_left: Dict[int, List[Tuple[int, float]]] = defaultdict(list)
        by_right: Dict[int, List[Tuple[int, float]]] = defaultdict(list)
        ...
        for (i, j), _ in sorted(matrix.items(), key=lambda item: item[1], reverse=True):
            ...
            col_margin = col[0][1] - col[1][1] if len(col) > 1 else 1.0
            item = details[(i, j)]
            if not self._cross_accept(item, float(row_margin), float(col_margin)):
                continue
    ...
    def _assign(self, components):
        ...
        for component in components:
            ...
            used.add(gid)
            self.registry.save_component(gid, model_banks=self._flat_gallery(component), cameras={group.camera for group in component}, last_ts=max(group.end for group in component), obs=len([key for group in component for key in group.members]))
            gallery = self.registry.load_gallery()
            text = f"G{gid:06d}"
            for group in component:
                for key in group.members:
    ...
    def resolve(self, local_mapping: Mapping[str, str], tracks: Mapping[str, object], cameras: List[str]):
        ...
        all_edges = same_edges + cross_edges
        return mapping, component_output, all_edges
