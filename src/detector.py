...
class Detection:
    ...
    # COCO-17 pose keypoints for this person: (17, 3) of (x, y, confidence), or
    # None with a box-only checkpoint.
    #
    # Captured at DETECTION time because they come out of the same forward pass
    # as the box -- nothing downstream can recover them from a crop. None is the
    # fail-open signal every consumer honours, so a box-only model behaves
    # exactly as before.
...
# Fixed by the COCO pose head's output shape. Not configurable.
NUM_KEYPOINTS = 17


def visible_keypoint_fraction(det, min_conf=0.5):
    """Fraction of COCO-17 joints on `det` scoring at least `min_conf`.

    None when there are no usable keypoints -- a box-only checkpoint, or a head
    that emits (x, y) with no confidence channel. Consumers treat None as "no
    opinion" and fall open.

    WHAT IT MEASURES. Keypoint confidence is the pose head's certainty about a
    joint's LOCATION. That correlates with visibility but is not a visibility
    flag: a visible joint scores low if it is small, blurred or oddly posed.
    Measured on this deployment, the per-camera medians (cam_213 0.41, cam_219
    0.65-0.76, cam_224 0.76) track median CROP HEIGHT (250 px / 622 px / large)
    almost exactly -- so on small distant people this reads distance, not
    occlusion. Treat it as a soft quality signal, weight before you gate, and
    never apply one global floor across cameras.

    THE OFFICE CASE. COCO-17 is nose, 2 eyes, 2 ears, 2 shoulders, 2 elbows,
    2 wrists, 2 hips, 2 knees, 2 ankles. Seated at a desk loses knees and ankles
    -> 13/17 = 0.76. Standing behind a desk -> 15/17 = 0.88. A "60-70% visible"
    rule therefore cuts straight through the seated population.
    """
    kp = getattr(det, "keypoints", None)
    if kp is None:
        return None
    arr = np.asarray(kp, dtype=np.float32)
    # (K, 2) means no per-joint confidence, so there is nothing to threshold.
    # Returning 1.0 would read perfect for every crop while measuring nothing.
    if arr.ndim != 2 or arr.shape[1] < 3:
        return None
    return float((arr[:, 2] >= float(min_conf)).sum()) / float(NUM_KEYPOINTS)
...
class PersonDetector:
    ...
    def _parse_results(self, results):
        ...
        for result in results:
            ...
            # A *-pose checkpoint returns keypoints INDEX-ALIGNED with boxes:
            # keypoints.data is (N, K, 3) against N boxes. A box-only checkpoint
            # lacks the attribute or sets it None -- versions differ, so never
            # assume it exists.
            kp_data = None
            ...
            if kp is not None and getattr(kp, "data", None) is not None:
                try:
                    ...
                    kp_data = np.asarray(kp.data)
                if kp_data.shape[0] != len(result.boxes):
                    # A wrong pairing is worse than none: it would attach one
                    # person's joints to another's box and every quality reading
                    # downstream would describe the wrong body.
                    kp_data = None
            ...
            for i, box in enumerate(result.boxes):
                ...
                detections.append(
                    Detection(
                        ...
                        class_id=class_id,
                        track_id=track_id,
                        keypoints=(None if kp_data is None else kp_data[i]),
                    )
                )
