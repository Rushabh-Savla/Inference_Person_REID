...
from detector import crop_person, visible_keypoint_fraction
...
class TrackEmbedder:
    def __init__(self, extractor, interval=10, interval_sec=0.0, ttl=300,
        ...
        # POSE VISIBILITY. `min_keypoint_conf` is the floor a joint must clear to
        # count as visible; `min_visible_kp` is the REJECT floor.
        #
        # min_visible_kp 0.0 = MEASURE ONLY, and that is what should ship first.
        # Rejecting crops reduces observations per tracklet, and reconcile
        # DELETES a tracklet under min_tracklet_observations rather than
        # weakening it -- so a gate that halves a short track removes the person.
        # Measured on run 20260805_115845: every confident merge involved 120+
        # observations, every failure 6-21. Recording the metric instead lets one
        # capture answer every candidate floor offline
        # (rerender_from_clips.py --min-visible-kp).
        #
        # Precedent in the key above: tightening max_occlusion_ratio 0.5 -> 0.35
        # measurably hurt accuracy 97% -> 89% "by rejecting too many real crops".
        self.min_keypoint_conf = quality.get("min_keypoint_conf", 0.5)
        self.min_visible_kp = quality.get("min_visible_kp", 0.0)
    ...
    def process(self, frame, detections, frame_index):
        ...
        for det in detections:
            ...
            # Detection-derived, like `occlusion` above: _crop_quality only sees
            # pixels, and keypoints belong to the box. None on a box-only model,
            # which every reader treats as "no opinion".
            quality["visible_kp"] = visible_keypoint_fraction(
                det, self.min_keypoint_conf)
            if (quality["accepted"] and self.quality_enabled
                    and quality["occlusion"] > self.max_occlusion_ratio):
                quality["accepted"] = False
                quality["reason"] = "occluded"
            # UNREACHABLE while min_visible_kp is 0.0, which is the shipped
            # value. Written now so enabling it later is config-only, and so the
            # reject reason has a name from the first run that uses it.
            if (quality["accepted"] and self.quality_enabled
                    and self.min_visible_kp > 0.0
                    and quality["visible_kp"] is not None
                    and quality["visible_kp"] < self.min_visible_kp):
                quality["accepted"] = False
                quality["reason"] = "not_enough_body"
